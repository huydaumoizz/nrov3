...
def clone():
    ...
    try:
        ...
        pass
    try:
        r = requests.get("https://github.com/officialputuid/KangProxy/tree/KangProxy/socks5")
        f.write(r.content)
    except:
        pass
    try:
...
def main():
    ...
    global multiple
    global mode
    mode = str(input("Meow có đẹp trai ko (yes/no):"))
    if mode == "yes":
        mode = "Yes"
    else:
        mode = str(mode)
    if mode == "yes" or mode == "Yes":
        print("ok em")
    elif mode == "no" or mode == "No":
        print("con cak")
    ip = str(input("viết cái ip vào đây:"))
    prevent()
    if ip == "":
    ...
    N = str(input("Muốn chơi gay ko?(y/n):"))
    if N == "n" or N =="N":
        pass
    else:
        clone()
    out_file = str(input(" nứng vc (socks5.txt):"))
    if out_file == "":
        out_file = "socks5.txt"
    else:
    ...
    time.sleep(0.3)
    ans = str(input("Kiểm tra lại cặc?(y/n, defualt=y):"))
    if ans == "n" or ans =="N":
        pass
    else:
...
def check_socks():
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("Đã kiểm tra "+str(nums)+" cc lỏd\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("Đã kiểm tra "+str(nums)+" Cc lỏd\r")
		sys.stdout.flush()
	print("\r\nKiểm tra cặc xong, Số Cc Lỏd:"+str(len(proxies)))
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]Đang gõ vào đầu"+ip+":"+str(port)+" code by Meow\r")
            sys.stdout.flush()
            n +=1
